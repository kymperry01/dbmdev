#' @title Predict forward incremental temperature-based development of diamondback moth
#' based on supplied parameters
#'
#' @description Predicts forward-in-time temperature-dependent development for an
...
#' The returned data.frame will only contain the rows/timepoints where `TotalDev` < 1.
#'
...
#'
#' @export
fwdBriere <- function(df, a, Tmin, Tmax, m, timeInt = 60){
  obs <- df$obs
  ...
  mutate(df, dev = dev, totalDev = cumsum(dev)) %>%
    filter(totalDev < 1)
}
