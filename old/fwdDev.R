#' @title Predict forward temperature-based development of diamondback moth
#' over multiple lifestages and generations based on supplied parameters
#'
#' @description Predicts forward-in-time temperature-dependent development for multiple
#' consecutive DBM lifestages and generations based on supplied parameters. A separate
#' analagous function, \code{\link{revDev}}, predicts reverse-in-time development.
#'
#' @details Implements Briere's temperature-dependent insect development model,
#' equation 2 (Briere 1999) forward-in-time. Outputs DBM development for each time
...
#'
#' # create example dataframe with insect development parameters
...
#'
#' @export
fwdDev <- function(tempObsDf, devParamsDf, startDate, startTime = 12,
                   startStage, startDev, gens = 1, output = "increments", timeInt = 60, ...){
  # error checks
  ...
  df <- tempObsDf %>% filter(datetime >= startdt)
  allStages <- rownames(devParamsDf)
  ...
  if (gens >= 2) {
    ...
    stages <- rownames(devParamsDf)
    out <- vector("list", length(stages))
    names(out) <- stages

    for (g in seq(gens)[-1]) {# loop over gens excluding gen == 1
      for (s in stages){
        tmpDf <- filter(newDf, !datetime %in% fitted)
        out[[s]] <- do.call(fwdBriere, c(list(df = tmpDf),
                                         devParamsDf[s, ]))
  ...
  # error check: output is complete
  checkStages <- unique(gensOut[[gens]]$stage)
  endStage <- gensOut[[gens]] %>% filter(stage == max(checkStages))
  endDev  <- sum(endStage$dev) # dev in last stage should be close to 1
  if (length(checkStages) != length(stages) | endDev < 0.95) {
    stop("Predicted dates will exceed time series")
  }
